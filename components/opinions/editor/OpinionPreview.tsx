...
// Live preview konten Markdown untuk editor opinions
// Client Component — render di browser, bukan server
// Logika identik dengan OpinionContentRenderer tapi 'use client' karena digunakan dalam editor
...
const ChartBlock = dynamic(() => import('@/components/artikel/ChartBlock'), {
  ssr: false,
  loading: () => (
    <div className="my-10 w-full h-40 bg-primary-dark/5 animate-pulse border border-primary-dark/10 flex items-center justify-center">
      <span className="font-helvetica text-xs text-primary-dark/30">Memuat chart...</span>
    </div>
  ),
})
...
export default function OpinionPreview({ content, charts }: OpinionPreviewProps) {
  if (!content.trim()) {
    return (
      <div className="px-6 py-12 text-center">
        <p className="font-helvetica text-sm text-primary-dark/30">
          Preview akan muncul di sini saat kamu mulai menulis.
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="article-content prose prose-lg max-w-none font-libre text-primary-dark px-6 py-5">
      <ReactMarkdown
        ...
        components={{
            ...
            if (config) {
              return (
                <div className={`my-8 border-l-4 ${config.borderClass} ${config.bgClass} px-5 py-4 rounded-r`}>
                  <p className={`font-helvetica font-bold text-xs uppercase tracking-widest mb-2 ${config.labelClass}`}>{config.label}</p>
                  <div className="font-libre text-base text-primary-dark/90 [&>p]:mb-0 [&>p]:leading-relaxed">{children}</div>
                </div>
              )
            }
            return (
          ...
          code: ({ className, children, ...props }) => {
            ...
            return match ? (
              ...
              <code className="font-mono text-sm bg-primary-dark/5 px-1.5 py-0.5 rounded" {...props}>{children}</code>
            )
          },
          ...
          img: ({ src, alt }) => {
            ...
            return (
              <figure className="my-10">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={src} alt={caption || ''} className="w-full rounded border border-primary-dark/10" />
                {caption && (
                  <figcaption className="text-center mt-3 text-sm text-primary-dark/60 font-helvetica">
                    {caption}{source && <span className="block text-xs mt-1">Sumber: {source}</span>}
                  </figcaption>
                )}
              </figure>
            )
