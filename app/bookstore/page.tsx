...
export default async function BookstorePage() {
  ...
  // Query Skema Baru: Ambil buku berserta relasi penulis dan varian produk
  const { data: books, error } = await supabase
    ...
    .select(`
      ...
      cover_image_url,
      authors ( name ),
      book_variants ( id, format, price_amount, stock_qty, is_active )
    `)
    .eq('status', 'active')
    ...
    <main className="min-h-screen bg-surface-page">
      <div className="max-w-5xl mx-auto px-5 py-12 md:py-16">
        <header className="mb-10 text-center">
          <Badge variant="kicker" className="mb-3">Bookstore</Badge>
          <h1 className="font-display text-4xl font-bold text-text-primary leading-tight">Katalog Buku</h1>
          <p className="font-interface text-base text-text-secondary mt-4 max-w-xl mx-auto">
            Buku kurasi redaksi. Temukan perspektif struktural dan angka yang tak sekadar narasi.
          </p>
        </header>
        ...
        {items.length === 0 && !error ? (
          <div className="py-20 text-center border border-border-default/20 rounded-md bg-surface-elevated">
            <p className="font-interface text-text-secondary">Katalog buku sedang kosong. Kami akan segera memperbaruinya.</p>
          </div>
        ) : (
